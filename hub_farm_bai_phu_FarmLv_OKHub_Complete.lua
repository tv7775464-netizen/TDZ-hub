    ...
    if enemies then
        for _, mob in ipairs(enemies:GetChildren()) do
            if mob.Name ~= mainEnemyName and Attack.Alive(mob) then
                ...
                if hrp then
                    FarmLvTP(hrp.CFrame * CFrame.new(0, 20, 0))
                    repeat
                        task.wait(Sec)
    ...
    if spawnFolder then
        for _, spawnPoint in ipairs(spawnFolder:GetChildren()) do
            if spawnPoint:IsA("BasePart")
                ...
                FarmLvTP(spawnPoint.CFrame * CFrame.new(0, 20, 0))
                return true
            end
        end
    end
...
local FarmLvTPToken = 0
local FarmLvCurrentTween = nil
local alreadyTeleported = false
local teleporting = false

local function StopFarmLvTP()
    FarmLvTPToken = FarmLvTPToken + 1
    if FarmLvCurrentTween then pcall(function() FarmLvCurrentTween:Cancel() end) end
    FarmLvCurrentTween = nil
    teleporting = false
    alreadyTeleported = false
    pcall(function() shouldTween = false end)
end

local function FarmLvTP(target)
    if not _G.Level or not target then return nil end
    local token = FarmLvTPToken
    if FarmLvCurrentTween then pcall(function() FarmLvCurrentTween:Cancel() end) end
    local tween = _tp(target)
    FarmLvCurrentTween = tween
    if tween then
        task.spawn(function()
            while tween.PlaybackState == Enum.PlaybackState.Playing do
                if not _G.Level or token ~= FarmLvTPToken then
                    pcall(function() tween:Cancel() end)
                    return
                end
                task.wait(0.05)
            end
            if FarmLvCurrentTween == tween then FarmLvCurrentTween = nil end
        end)
    end
    return tween
end
...
FarmLevel = Tabs.Main:AddToggle({
    ...
    Callback = function(Value)
        _G.Level = Value
        FarmLvTPToken = FarmLvTPToken + 1
        if not Value then StopFarmLvTP() end
        alreadyTeleported = false
        teleporting = false
    end
})
...
local function IsInSubmergedIsland()
    ...
    return (Vector3.new(hrp.Position.X,0,hrp.Position.Z)-islandXZ).Magnitude < 2000
end
    ...
    while task.wait(Sec or 0.1) do
        if _G.Level then
            pcall(function()
                ...
                if not Root or not _G.Level then return end
                ...
                local enemyName, questId, questName, enemyPos, requiredTitle, questPos = q[1],q[2],q[3],q[4],q[5],q[6]

                if questUI.Visible and requiredTitle and QuestTitle ~= "" and not string.find(QuestTitle,requiredTitle,1,true) then
                    replicated.Remotes.CommF_:InvokeServer("AbandonQuest")
                    return
                end

                if not questUI.Visible then
                    if questPos and _G.Level then
                        if (Root.Position-questPos.Position).Magnitude > 8 then
                            FarmLvTP(questPos)
                ...
                if enemies then
                    for _,mob in ipairs(enemies:GetChildren()) do
                        if mob.Name == enemyName and Attack.Alive(mob) then target=mob break end
                    end
                end

                if target then
                    repeat
                        if not _G.Level or not target.Parent then break end
                    ...
                    local secondary = FarmSecondaryWhileWaiting(enemyName)
                    if not secondary and _G.Level and enemyPos and (Root.Position-enemyPos.Position).Magnitude > 100 then
                        FarmLvTP(enemyPos)
                    end
                end
            end)
